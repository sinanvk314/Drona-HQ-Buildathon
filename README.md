...
- **Real email.** Is real email working, and what happened to each message: test the Gmail connection, send one test email, check the inbox now, and see every real message with its delivery result or the reason it failed. Start here when something does not arrive.
