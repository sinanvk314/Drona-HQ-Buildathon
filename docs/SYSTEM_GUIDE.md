...
## 23. Real data and real sending

**One switch per campaign.** `sourcing` is `"real"`, `"simulated-search"` or `"synthetic"`. `isRealCampaign(c)` (`services/realMode.js`) is true only for `"real"` and never for a sandbox. That single test decides everything below; a campaign that is not real behaves exactly as it always did.

**Real contacts** (`services/contacts.js`) are the people entered by hand (`state.realContacts`, no migration needed). Discovery for a real campaign (`runDiscovery`) ranks them against the audience by word overlap and takes the best; nothing is searched for or invented, and no simulated reply is ever generated for a real prospect. A real single-person campaign uses the person typed on the form.

**Sending** happens in one place. `recordTouch` and `recordReply` (`services/outreach.js`) record the message, then call `dispatch` (`services/channels/dispatch.js`), which, for real campaigns only, checks `channelBlocker` and `recipientBlocker` (`realMode.js`: switch on, credentials present, real address, allow-list) and then sends through Gmail (`channels/gmail.js`), Twilio SMS or the Voice SDR (`channels/voice.js`). The touch and the conversation entry carry a `delivery` of `sending`, `sent` or `failed` with the reason, shown on the prospect page. A calendar invite is attached to the first email sent after a meeting is booked.

**Human in the loop.** For a real campaign `autoApproval` and `sendsItself` (`scheduler.js`) never skip a human unless `REAL_AUTO_SEND=on`. `outreachAllowed` holds a real campaign whose channel cannot really send, `decideApproval` refuses to approve one that cannot be sent, and the launch review blocks a real campaign with no usable channel. LinkedIn has no sending path, and the Strategy agent is given only channels that can really send now (and never `voice` in a simulated campaign).

**Replies.** Email: `pollInbox` (`scheduler.js`, every `GMAIL_POLL_MS`) reads each contacted person's Gmail thread, trims quoted text, and hands each new message to `processReply` like any typed reply. Texts and calls come to `/webhooks/twilio/*` (`routes/webhooks.js`), outside sign-in, and every request must carry a valid Twilio signature (`validSignature`), rebuilt from `PUBLIC_URL`.

**The Voice SDR** places the call in `startCall` (the opening is decided first, so it is ready when the person answers). Twilio then calls `/voice` (say the opening and listen), `/voice/turn` (the person spoke: `engine.voiceTurn` decides the reply or ends the call, at most six turns) and `/voice/status` (the call ended: `completeCall` in `scheduler.js` writes the transcript into the conversation, puts an opt-out on the do-not-contact list, or asks a human to book a meeting). `voiceTurn` uses Gemini with a plain rule fallback, like every other agent.

**Audience kind.** `audienceKind` is `"organisations"` (default) or `"individuals"`. For individuals the search prompt (`SOURCE_INDIVIDUALS_SYSTEM`) may name well-known public figures using only widely known facts, no organisation is required, and every email is still a made-up `.example` address. A real contact of any kind is only ever what a person typed in.

**Tests.** `test/real.test.js` runs all of this against fake Gmail and Twilio servers.

**Email details worth knowing.** A message is ours only if its id is in `prospect.seenMessageIds` (the ids of messages we sent through the API), never because of its From address: when the person is the same Gmail account as the sender, their reply comes from that same address and is still a reply. Bounces (a mailer-daemon message) mark the address as not working and stop; automatic replies (`Auto-Submitted`, an out-of-office subject) are noted and never answered. Quoted text is cut however the person's mail program marks it, and HTML-only replies are read. Every real email gets a professional close (`Best regards,` and the campaign sign-off or the rep's name), an opt-out line and a `List-Unsubscribe` header. Approving any drafted reply to a real person (an answer, a meeting proposal, an escalation) is what sends it, through `dispatch`. The Dev tab's Real email panel (`services/dev.js`: `getEmailStatus`, `testEmailConnection`, `sendTestEmail`, `checkInboxNow`) asks Gmail directly and shows what it answers.

**When the model does not answer.** Every agent call goes through `withFallback` (`agentEngine/index.js`), which returns the rule engine's answer with a `fallbackReason` when the model failed. The scheduler passes it to `pushDecision` and `addNote`: the Decision Journal entry gets an evidence line ("The AI did not respond (...), so the rule engine decided this step"), the prospect's log (`history`) gets the same line, and the hand-off note keeps it in `note.fallback`. Two things are never done on a guess while a model is configured but failing: the AI people search adds nobody (the free generator is used only when no model is configured at all), and a prospect whose audience the rules cannot judge waits and is retried every minute instead of being rejected. The last model error is on Dev, Runtime (with an "Ask Gemini a question" button) and on the Gemini card in Settings.
