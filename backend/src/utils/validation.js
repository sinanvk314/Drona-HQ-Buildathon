// Ported 1:1 from the frontend's src/utils/validation.js so client- and server-side
// validation agree exactly.
export function validateCampaign(v, launch) {
  ...
  if (!v.channels || !v.channels.length) e.channels = "Select at least one channel.";
  if (!v.qualificationPrompt || !v.qualificationPrompt.trim()) e.qualificationPrompt = "Add the qualification criteria.";
